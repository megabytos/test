...
//            $('div.table-caption').remove();
    ...
    $('#fast_inp_1').prop('checked', Boolean(hasEdgeDetails));
    if ($('#fast_inp_1').prop('checked')) {
        table_type = 1;
    }
}
...
function sendArray(callback) {
    $.ajax({
        ...
        data: {controller: 'Cutting', action: 'addArray', data: details, forceStol: 1},
        dataType: 'json',
...
function open2() {
    ...
    processDetails(function (data) {
        ...
        if($(".mi-mi-table tr th").length==8||$(".mi-mi-table tr th").length==12){
...
function second(options) {
    ...
    table2.setIdxCaption('key')
        ...
        filter: true,
        textWithValidate: true,
        disable: materialType == 'compact'? false : true,
        default: mainHeight,
        validate: function (data) {
            return validateForCalculate(data.width);
        }
    ...
    if (table_type == 2 || table_type == 3) {
        ...
        //     errorMsg: 'Невозможно учесть текстуру для указанных габаритов детали!<span><a data-action="help" data-section="details#texture" class="go-to-help">Справка</a></span>'
        // })
            .addCell({
                type: 'caption',
                edit: true,
                text: '<a data-action="help" data-section="details#caption" class="help">Название</a>',
                input: {
                    tag: 'input',
                    type: 'text',
                    color: 'white'
    ...
    inputTable2 = table2.addAction('help', function (event) {
        ...
        .change(function (data, serverKeyFunc) {
            ...
            if (autosave) {
                ...
                details[data['key']] = {data: result, edges: edges};
...
function table_start() {
    $.ajax({
        type: "POST",
        url: "/service/system/controllers/JsonController.php",
        data: ({controller: 'Cutting', action: 'getMainMaterial'}),
        dataType: 'json',
        async: true,
        success: function (val) {
            mainHeight = !val.isCompact ? val.height : '';
            tableStartAfter();
        }
    });
}

function tableStartAfter() {
    var checks = {
